...
RaceNote의 기본 라이트 테마를 뉴트럴 그레이와 코랄 조합으로 확정하고, 사용자가
라이트/다크 모드를 전환해 선택을 저장할 수 있게 한다.
...
- 크림 또는 웜 그레이 주요 텍스트
- 낮은 대비의 보조 텍스트와 경계선
- 라이트 테마보다 부드러운 코랄 포인트

코랄은 활성 필터, 포커스 링, 링크 호버, 작은 마커, 주요 액션에만 사용한다. 넓은
배경 전체에는 사용하지 않는다. F1, WEC, WRC 고유색은 유지하되 전체 팔레트에 맞춘
현재 저채도 방향을 유지한다.
