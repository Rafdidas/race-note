...
1. `docs/superpowers/plans/2026-06-17-f1-race-detail.md`의 Task 1부터 F1 레이스
   상세 강화를 구현(DB 스키마·마이그레이션 0006 → 공개 변환·조회·UI → 관리자
   탭·입력). 원격 D1 적용·배포는 사용자 명시 요청 시에만.
2. 관리자 레이스 목록의 F1/WEC/WRC, 검수 필요, 공개/비공개 필터를 실제 조회 또는
   화면 동작에 연결
3. 관리자 수동 일정 수집의 실행 중 버튼 비활성화·로딩 표시와 로그 없음 상태 보완
4. 향후 주요 레이스부터 세션, Must Watch, AI 문구를 검수·공개
5. 공개/비공개, KST, 모바일, 라이트·다크 테마, 관리자 인증, AI 검수·공개 흐름,
