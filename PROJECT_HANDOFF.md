...
1. 관리자 레이스 목록의 F1/WEC/WRC, 검수 필요, 공개/비공개 필터를 실제 조회 또는
   화면 동작에 연결
2. 관리자 수동 일정 수집의 실행 중 버튼 비활성화·로딩 표시와 로그 없음 상태 보완
3. 향후 주요 레이스부터 세션, Must Watch, AI 문구를 검수·공개
4. 공개/비공개, KST, 모바일, 라이트·다크 테마, 관리자 인증, AI 검수·공개 흐름,
