...
추가 개발 Phase 1(F1 레이스 상세 강화: Quick Facts/History/Who to watch/Next race/
Explore)도 구현·원격 D1 적용·배포까지 완료했습니다. 신규 섹션은 F1 상세에서만
노출하며, 실데이터는 관리자 입력 후 채워집니다(아래 운영 작업). 다음 개발 예정
사항은 이 절 끝의 "다음 예정 개발 로드맵"을 기준으로 합니다.
...
- "완성에 가까운데 볼 게 없는 사이트"라는 문제 인식에서, `racenote_mvp_design_v2.md`의
  Phase 1(레이스 상세 강화)을 다음 작업으로 정했습니다.
...
1. 관리자 로그인 후 주요 공개 F1 레이스(예: `f1-2026-round-7`)에 Quick Facts/
   History/Watch Targets를 입력하고 공개 상세에서 실데이터 렌더를 확인. (Phase 1을
   사용자에게 보이게 하는 운영 작업 — 코드 변경 없음.)
2. 미커밋 상태인 Phase 1 변경 전체를 커밋(현재 워킹 트리에만 있음). 새 브랜치 또는
   사용자 지정 방식으로.
3. 관리자 레이스 목록의 F1/WEC/WRC, 검수 필요, 공개/비공개 필터를 실제 조회 또는
   화면 동작에 연결.
4. 관리자 수동 일정 수집의 실행 중 버튼 비활성화·로딩 표시와 로그 없음 상태 보완.
5. 향후 주요 레이스부터 세션, Must Watch, AI 문구를 검수·공개.
6. 공개/비공개, KST, 모바일, 라이트·다크 테마, 관리자 인증, AI 검수·공개 흐름,
   SEO 메타, Cron 로그를 포함한 배포 전 최종 QA.

## 다음 예정 개발 로드맵

`racenote_mvp_design_v2.md`의 단계별 라우팅(4.3)을 기준으로 한다. 현재까지 완료한
Phase 1은 "레이스 상세 강화"(facts/history/watch/next/explore)이며, 그다음은 F1
중심의 시리즈·순위·팀·드라이버 확장이다.

- **다음(1차): F1 시리즈 메인 + 현재 순위** — 라우팅 `/series/f1`,
  `/series/f1/standings`. 드라이버/컨스트럭터 순위 TOP N, 다음 경기 요약. 필요
  작업: 순위 데이터 소스 확보(공식/Jolpica), `drivers`/`teams`/`standings` DB
  테이블 신설(다음 마이그레이션 번호 `0007`), 수집 또는 관리자 입력 경로, 공개
  표시. 레이스 상세의 "현재 순위 요약 / 유력 승리자"도 이 데이터와 연결 가능.
- **1.5차: F1 팀·드라이버 목록** — `/series/f1/teams`, `/series/f1/drivers`.
- **2차: 팀·드라이버·머신 상세** — `/series/f1/teams/[slug]`,
  `/series/f1/drivers/[slug]`, `/series/f1/cars/[slug]`. Watch Target의 `target_id`
  FK 연결(현재 Phase 1은 직접 입력만)을 이 단계에서 검토.
- **3차: WEC/WRC 상세 확장** — `src/lib/public-data.ts`의 `ENRICHED_DETAIL_SERIES`에
  시리즈 추가 + `RaceFacts` 공개/관리자 컴포넌트를 시리즈별 표시 필드로 분기(랠리는
  스테이지/노면, 내구는 클래스/시간 등). 스키마·관리자 입력은 이미 시리즈 범용이라
  공개 노출 게이트와 레이아웃만 확장하면 된다.

1차 제외(추후): 실시간 경기 순위, 팀/드라이버 이미지, 일반 회원 기능(MVP 제외 유지).
